...
def main():
    ...
    n, m = [int(num) for num in file.readline().split(" ", 1)]
    #2 scenarios D
    D1 = np.array([int(num) for num in file.readline().split(" ", n-1)])
    D2 = np.array([int(num) for num in file.readline().split(" ", n-1)])
    print(D1)
    print(D2)
    #vector b: preorder cost bj per unit of part j
    b = np.array([int(num) for num in file.readline().split(" ", m-1)])
    print(b)
    #vector s: savage value where sj < bj
    s = np.array([int(num) for num in file.readline().split(" ", m-1)])
    print(s)
